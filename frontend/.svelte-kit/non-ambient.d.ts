...
declare module "$app/types" {
	export interface AppTypes {
		RouteId(): "/(user)" | "/(registration)" | "/(app)" | "/" | "/admin" | "/admin/masters" | "/admin/profile" | "/admin/users" | "/admin/workshops" | "/admin/workshops/[id]" | "/(registration)/login" | "/(registration)/logout" | "/(user)/orders" | "/(user)/orders/[id]" | "/(user)/profile" | "/(user)/schedule" | "/(user)/schedule/[id]" | "/(registration)/signup";
		RouteParams(): {
			"/admin/workshops/[id]": { id: string };
